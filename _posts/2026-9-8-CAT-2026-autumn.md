---
title: 计算机辅助翻译
author: Huang Jie
date: 2026-09-08
layout: post
---

<!-- 课程简介区块 -->
<div style="background:#f5f7fa; border-radius:8px; padding:20px 30px; margin:24px 0 32px 0; font-size:1.1em; line-height:2.2;">
<strong>课程：</strong>计算机辅助翻译（本科）<br>
<strong>授课学期：</strong>2026秋<br>
<strong>教师：</strong>黄婕<br>
<strong>授课单位：</strong>对外经济贸易大学英语学院<br>
<strong>转载请注明出处：</strong>https://chezvivian.github.io/class<br>
<strong>历史学期完整课件：</strong><a href="/class/archive/CAT-2026-spring/" target="_blank">2026春《计算机辅助翻译AI+》课件</a>
</div>

<!-- 智能AI助教美化区块（浅灰理科风格） -->
<div style="background:#f5f7fa; border-radius:14px; box-shadow:0 2px 12px #e0e0e0; padding:28px 32px; margin:32px 0 36px 0; display:flex; align-items:center; gap:32px;">
  <div style="flex:1;">
    <div style="font-size:1.35em; font-weight:bold; color:#2d3a4a; margin-bottom:10px; letter-spacing:1px;">
      <span style="font-size:1.5em;">🤖</span> 智能AI助教 <span style="font-size:1.1em; color:#5b7fa6;">CAT课程专属</span>
    </div>
    <div style="margin-bottom:10px;">
      <a href="https://udify.app/chat/cowvutHCzOFeVfaw" target="_blank" style="background:#e3e8ef; color:#2d3a4a; padding:8px 18px; border-radius:6px; font-size:1.1em; text-decoration:none; font-weight:bold; box-shadow:0 2px 6px #b3b3b3; transition:background 0.2s;">🌐 网页端一键进入</a>
    </div>
    <div style="font-size:1em; color:#6c7a89;">手机端请扫码右侧二维码</div>
  </div>
  <div style="flex-shrink:0;">
    <img src="https://chezvivian.github.io/class/assets/CAT助教_2.0.png" alt="智能助教二维码" style="width:180px; border-radius:10px;">
  </div>
</div>

<script>
 window.difyChatbotConfig = {
  token: 'cowvutHCzOFeVfaw'
 }
</script>
<script
 src="https://udify.app/embed.min.js"
 id="cowvutHCzOFeVfaw"
 defer>
</script>
<style>
  #dify-chatbot-bubble-button {
    background-color: #e3e8ef !important;
  }
  #dify-chatbot-bubble-window {
    width: 40rem !important;
    height: 45rem !important;
  }
</style>

<!-- 学期计划与资源表格区块（冷静配色，宽度100%） -->
## 学期计划与资源

<table class="cat-table" style="width:100%; border-collapse:collapse; text-align:center; font-size:1em; background:#fff;">
  <tr style="background:#e3e8ef; color:#2d3a4a;">
    <th style="border:1px solid #e0e0e0; padding:10px 24px;">课次</th>
    <th style="border:1px solid #e0e0e0; padding:10px 40px;">主题与课件</th>
    <th style="border:1px solid #e0e0e0; padding:10px 12px; font-size:0.8em;">简介</th>
    <th style="border:1px solid #e0e0e0; padding:10px 24px;">模块</th>
  </tr>
  <tr style="background:#fff8e1; color:#2d3a4a;">
    <td style="border:1px solid #e0e0e0; padding:10px;">第1讲</td>
    <td style="border:1px solid #e0e0e0; padding:10px 40px;"><a href="/class/CAT_pdf/2026autumn/Week1_introduction.pdf" target="_blank">计算机辅助翻译技术与工具概况</a></td>
    <td style="border:1px solid #e0e0e0; padding:10px 12px; font-size:0.8em;">概述CAT的发展历程、基本原理和主要工具类型，帮助学生建立整体认知。</td>
    <td style="border:1px solid #e0e0e0; padding:10px 24px;" rowspan="1"><b>全景与思维</b></td>
  </tr>
  <tr style="background:#eaf4fb; color:#2d3a4a;">
    <td style="border:1px solid #e0e0e0; padding:10px;">第2讲</td>
    <td style="border:1px solid #e0e0e0; padding:10px 32px;"><a href="/class/CAT_pdf/2026autumn/Week2_Word在翻译工作中的应用.pdf" target="_blank">Word在翻译工作中的应用</a></td>
    <td style="border:1px solid #e0e0e0; padding:10px 12px; font-size:0.8em;">介绍Word在翻译流程中的常用功能和技巧，提高效率和质量。</td>
    <td style="border:1px solid #e0e0e0; padding:10px 24px;" rowspan="4"><b>译前准备技术</b></td>
  </tr>
  <tr style="background:#eaf4fb; color:#2d3a4a;">
    <td style="border:1px solid #e0e0e0; padding:10px;">第3讲</td>
    <td style="border:1px solid #e0e0e0; padding:10px 32px;"><a href="/class/CAT_pdf/2026autumn/Week3_文件格式转换与桌面排版.pdf" target="_blank">文件格式转换与桌面排版</a></td>
    <td style="border:1px solid #e0e0e0; padding:10px 12px; font-size:0.8em;">讲解文件格式转换与桌面排版要点，解决翻译中的格式兼容与版式问题。</td>
  </tr>
  <tr style="background:#eaf4fb; color:#2d3a4a;">
    <td style="border:1px solid #e0e0e0; padding:10px;">第4讲</td>
    <td style="border:1px solid #e0e0e0; padding:10px 32px;"><a href="/class/CAT_pdf/2026autumn/Week4_信息检索技术.pdf" target="_blank">信息检索技术</a></td>
    <td style="border:1px solid #e0e0e0; padding:10px 12px; font-size:0.8em;">介绍信息检索的基本原理和常用方法，提升查找资料和术语能力。</td>
  </tr>
  <tr style="background:#eaf4fb; color:#2d3a4a;">
    <td style="border:1px solid #e0e0e0; padding:10px;">第5讲</td>
    <td style="border:1px solid #e0e0e0; padding:10px 32px;">语料库工具及语料库创建入门</td>
    <td style="border:1px solid #e0e0e0; padding:10px 12px; font-size:0.8em;">讲解语料库的基本概念、常用工具及创建流程。</td>
  </tr>
  <tr style="background:#fef0e6; color:#2d3a4a;">
    <td style="border:1px solid #e0e0e0; padding:10px;">第6讲</td>
    <td style="border:1px solid #e0e0e0; padding:10px 32px;">CAT基础：Trados翻译项目管理</td>
    <td style="border:1px solid #e0e0e0; padding:10px 12px; font-size:0.8em;">介绍Trados在翻译项目管理中的基础流程与核心功能。</td>
    <td style="border:1px solid #e0e0e0; padding:10px;" rowspan="4"><b>CAT翻译项目管理</b></td>
  </tr>
  <tr style="background:#fef0e6; color:#2d3a4a;">
    <td style="border:1px solid #e0e0e0; padding:10px;">第7讲</td>
    <td style="border:1px solid #e0e0e0; padding:10px 32px;">CAT基础：YiCAT云端翻译项目管理</td>
    <td style="border:1px solid #e0e0e0; padding:10px 12px; font-size:0.8em;">讲解YiCAT云端翻译项目管理的操作流程与协作方式。</td>
  </tr>
  <tr style="background:#fef0e6; color:#2d3a4a;">
    <td style="border:1px solid #e0e0e0; padding:10px;">第8讲</td>
    <td style="border:1px solid #e0e0e0; padding:10px 32px;">CAT进阶：术语管理流程与工具</td>
    <td style="border:1px solid #e0e0e0; padding:10px 12px; font-size:0.8em;">讲解术语管理流程及常用术语工具的应用方法。</td>
  </tr>
  <tr style="background:#fef0e6; color:#2d3a4a;">
    <td style="border:1px solid #e0e0e0; padding:10px;">第9讲</td>
    <td style="border:1px solid #e0e0e0; padding:10px 32px;">CAT进阶：译文质量控制</td>
    <td style="border:1px solid #e0e0e0; padding:10px 12px; font-size:0.8em;">介绍译文质量控制的基本概念、常用技术与工具。</td>
  </tr>
  <tr style="background:#eaf6ee; color:#2d3a4a;">
    <td style="border:1px solid #e0e0e0; padding:10px;">第10讲</td>
    <td style="border:1px solid #e0e0e0; padding:10px 32px;">机器翻译与大语言模型技术概论</td>
    <td style="border:1px solid #e0e0e0; padding:10px 12px; font-size:0.8em;">概述机器翻译与大语言模型的发展、原理及在翻译中的应用。</td>
    <td style="border:1px solid #e0e0e0; padding:10px;" rowspan="5"><b>人机协同实战</b></td>
  </tr>
  <tr style="background:#eaf6ee; color:#2d3a4a;">
    <td style="border:1px solid #e0e0e0; padding:10px;">第11讲</td>
    <td style="border:1px solid #e0e0e0; padding:10px 32px;">译后编辑策略与方法</td>
    <td style="border:1px solid #e0e0e0; padding:10px 12px; font-size:0.8em;">讲解机器翻译译文的后期编辑方法、常见问题及应对策略。</td>
  </tr>
  <tr style="background:#eaf6ee; color:#2d3a4a;">
    <td style="border:1px solid #e0e0e0; padding:10px;">第12讲</td>
    <td style="border:1px solid #e0e0e0; padding:10px 32px;">影视字幕和配音翻译技术</td>
    <td style="border:1px solid #e0e0e0; padding:10px 12px; font-size:0.8em;">介绍影视字幕和配音翻译的基本流程、技术要点及常用软件工具。</td>
  </tr>
  <tr style="background:#eaf6ee; color:#2d3a4a;">
    <td style="border:1px solid #e0e0e0; padding:10px;">第13讲</td>
    <td style="border:1px solid #e0e0e0; padding:10px 32px;">小组汇报1</td>
    <td style="border:1px solid #e0e0e0; padding:10px 12px; font-size:0.8em;">小组展示智能体工作流成果。</td>
  </tr>
  <tr style="background:#eaf6ee; color:#2d3a4a;">
    <td style="border:1px solid #e0e0e0; padding:10px;">第14讲</td>
    <td style="border:1px solid #e0e0e0; padding:10px 32px;">小组汇报2</td>
    <td style="border:1px solid #e0e0e0; padding:10px 12px; font-size:0.8em;">小组展示智能体工作流成果。</td>
  </tr>
</table>

## 课前准备（2026秋季学期-计算机辅助翻译）

<div style="background:#f9f6f2; border-left:6px solid #232a36; border-radius:8px; box-shadow:0 2px 8px #eee; padding:24px 24px 24px 32px; margin:32px 0;">
  <p style="font-style:italic; color:#666; margin-bottom:20px;">2026年9月8日</p>
  <p style="font-size:1.1em; line-height:1.8; color:#232a36;">
本学期课程内容有一定幅度的调整。修改了几次作业的内容，还调整了 CAT 工具的授课重点：在传统的 Trados 基础上，向云端工具 YiCAT 倾斜，并更新为新的作业。另外，小组汇报也从之前的 CAT 软件学习，更新为小组制作智能体工作流，更具创新性。<br><br>

新的学期，继续优化，继续迎接挑战！

  </p>
</div>
